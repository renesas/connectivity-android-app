...
public class AWSIoTDoorActivity extends Activity {
    ...
    //[[change in v2.4.11
    //private com.wang.avi.AVLoadingIndicatorView progressingConnecting;
    private ProgressBar progressingConnecting;
    //private static com.wang.avi.AVLoadingIndicatorView progressingOpenClose;
    ...
    private static boolean isAyncTaskCompleted = true;  //add in v2.3.16
    ...
    //[[add in v2.3.19 for update shadow example
    public UpdateShadowTask updateShadowTask;
    /*private EditText et_temperature;
    private Button btn_setTemperature;*/
    //]]
    ...
    public static AWSIoTDoorActivity getInstanceMain() {return instanceDoor;}

    static boolean toggleOpen = false;

    private final Handler handler = new Handler();  //add in v2.3.14
    ...
    public Handler DoorHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            switch(event) {
                ...
                //remove in v2.3.6
                    /*case E_STOP_GET_SHADOW:
                    if (StaticDataSave.thingName != null) {
                        MyLog.i("=== E_STOP_GET_SHADOW ===");
                        stopGetShadow();
                    }
                    break;*/
                //
                ...
                case E_UPDATE_SENSOR:
                    if (StaticDataSave.thingName != null) {
                        ...
                        //getSensorShadows();  //remove in v2.3.16
                        postEvent(AWSEVENT.E_UPDATE_SENSOR, UPDATE_SENSOR_TIME);
                    }
                    break;

                //remove in v2.3.6
                    /*case E_STOP_UPDATE_SENSOR:
                    if (StaticDataSave.thingName != null) {
                        MyLog.i("=== E_STOP_UPDATE_SENSOR ===");
                        stopGetSensorShadow();
                    }
                    break;*/
                //
                ...
                case E_SHOW_BATTERY_ALERT:
                    ...
                    if (batteryAlertDialog == null) {
                        ...
                        batteryAlertDialog.setCancelable(false);  //add in v2.3.15
                    }
                    break;
                ...
                case E_SHOW_TEMPERATURE_ALERT:
                    ...
                    if (temperatureAlertDialog == null) {
                        ...
                        temperatureAlertDialog.setCancelable(false);  //add in v2.3.15
                        break;
                    }
                ...
                case E_DEVICE_CONNECT_TIMEOUT:
                    ...
                    if (deviceConnectFailDialog == null) {
                        ...
                        deviceConnectFailDialog.setCancelable(false);  //add in v2.3.15
                    }
                    break;
            }
        ...
        mRunnable_mqtt_connect_noResponse = new Runnable() {
            ...
            public void run() {
                ...
                if (StaticDataSave.readyFlag == true && noInternetDialog == null) {
                    ...
                    noInternetDialog.setCancelable(false);  //add in v2.3.15
                }
        ...
        mRunnable_update_sensor_noResponse = new Runnable() {
            ...
            public void run() {
                ...
                if (noResponseDialog == null) {
                    ...
                    noResponseDialog.setCancelable(false);  //add in v2.3.15
                }
...
/*
        ...
        MyLog.i("keystorePath =  "+keystorePath);
        ...
        try {
            if (AWSIotKeystoreHelper.isKeystorePresent(keystorePath, keystoreName)) {
                if (AWSIotKeystoreHelper.keystoreContainsAlias(certificateId, keystorePath, keystoreName, keystorePassword)) {
                    MyLog.i("Certificate " + certificateId + " found in keystore - using for MQTT.");
                    // load keystore from file into memory to pass on connection
                    clientKeyStore = AWSIotKeystoreHelper.getIotKeystore(certificateId, keystorePath, keystoreName, keystorePassword);
                    connectMqtt();
                } else {
                    MyLog.i("Key/cert " + certificateId + " not found in keystore.");
                }
            } else {
                MyLog.i("Keystore " + keystorePath + "/" + keystoreName + " not found.");
            }
        } catch (Exception e) {
            MyLog.i("An error occurred retrieving cert/key from keystore : "+e);
        }


        if (clientKeyStore == null) {
            MyLog.i("Cert/key was not found in keystore - creating new key and certificate.");
            ...
            new Thread(new Runnable() {
                ...
                public void run() {
                    try {
                        ...
                        //[[live remove 220704
                        //policyAttachRequest.setPrincipal(awsConfig.CertificateArn);
                        //]]
                        ...
                        MyLog.e("Exception occurred when generating new private key and certificate. >>> " + e);
                        initMqtt();
                    }
                }
            }).start();
    ...
    public void connectMqtt() {
        ...
        //if (clientKeyStore != null) {  //remove condition in v2.3.16
        try {
            //[[modify in v2.3.16
    ...
    private void updateSensorRequest() {
        if (updateDialog == null) {
            ...
            updateDialog.setCancelable(false);  //add in v2.3.15
        }
    ...
    private void sensorStatusUpdated(String statusState) {
        ...
        if (dm.metadata.reported.temperature != null) {
            ...
            //[[modify in v2.4.13 for SMARTLOCK_BOARD
        ...
        if (dm.metadata.reported.OTAupdate != null) {
            ...
            if (isOtaUpdated) {
                ...
                if (ds.state.reported.OTAupdate == 0) {  //none
                    ...
                    runOnUiThread(new Runnable() {  //add thread in v2.3.17
                        @Override
                        public void run() {
    ...
    private void shadowUpdated(String statusState) {
        ...
        if (dm.metadata.reported.doorState != null) {
            //long curTimestamp = dm.metadata.reported.doorState.timestamp;
            Long curTimestamp = dm.metadata.reported.doorState.timestamp;  //change type from long to Long in v2.4.0
            if (curTimestamp != null) {  //add in v2.4.0
                Date dt = new Date (curTimestamp * 1000);
                SimpleDateFormat sfd = new SimpleDateFormat("yyyy-MM-dd hh:mm:ss aaa");
                updatedTimeText.setText(sfd.format(dt));
        ...
        if (dm.metadata.reported.temperature != null) {
            //long curTemperatureTimestamp = dm.metadata.reported.temperature.timestamp;
            Long curTemperatureTimestamp = dm.metadata.reported.temperature.timestamp;  //change type from long to Long in v2.4.0
            if (curTemperatureTimestamp != null) {  //add in v2.4.0
                Date dt2 = new Date (curTemperatureTimestamp * 1000);
                SimpleDateFormat sfd2 = new SimpleDateFormat("yyyy-MM-dd hh:mm:ss aaa");
                updatedTimeText.setText(sfd2.format(dt2));
        ...
        if (dm.metadata.reported.battery != null) {
            //long curBatteryTimestamp = dm.metadata.reported.battery.timestamp;
            Long curBatteryTimestamp = dm.metadata.reported.battery.timestamp;  //change type from long to Long in v2.4.0
            if (curBatteryTimestamp != null) {  //add in v2.4.0
                Date dt3 = new Date (curBatteryTimestamp * 1000);
                SimpleDateFormat sfd3 = new SimpleDateFormat("yyyy-MM-dd hh:mm:ss aaa");
                updatedTimeText.setText(sfd3.format(dt3));
        ...
        if (dm.metadata.reported.OTAupdate != null) {
            //long curOtaTimestamp = dm.metadata.reported.OTAupdate.timestamp;
            Long curOtaTimestamp = dm.metadata.reported.OTAupdate.timestamp;  //change type from long to Long in v2.4.0
            if (curOtaTimestamp != null) {  //add in v2.4.0
                Date dt4 = new Date (curOtaTimestamp * 1000);
                SimpleDateFormat sfd4 = new SimpleDateFormat("yyyy-MM-dd hh:mm:ss aaa");
                updatedTimeText.setText(sfd4.format(dt4));
                ...
                if (ds.state.reported.OTAupdate == 0) {  //none
                    ...
                    if (ds.state.reported.OTAresult != null) {
                        ...
                        if (ds.state.reported.OTAresult.equals("OTA_OK")) {
                            if(getRunActivity().equals("com.renesas.wifi.awsiot.AWSIoTDoorActivity")) {
                                runOnUiThread(new Runnable() {  //add thread in v2.3.15
                                    @Override
                                    public void run() {
                        ...
                        //[[remove in v2.3.17
                        /*else if (ds.state.reported.OTAresult.equals("OTA_NG")) {
                            if(getRunActivity().equals("com.renesas.wifi.awsiot.AWSIoTDoorActivity")) {
                                if (updatingDialog != null && updatingDialog.isShowing()) {
                                    runOnUiThread(new Runnable() {  //add thread in v2.3.15
                                        @Override
    ...
    private class GetShadowTask extends AsyncTask<Void, Void, AsyncTaskResult<String>> {
        ...
        protected AsyncTaskResult<String> doInBackground(Void... voids) {
            ...
            isAyncTaskCompleted = false;  //add in v2.3.16
        ...
        protected void onPostExecute(AsyncTaskResult<String> result) {
            ...
            isAyncTaskCompleted = true;  //add in v2.3.16
        }
    }
    ...
    /*private void setTemperature() {
        if (StaticDataSave.thingName != null) {
            if(updateShadowTask != null) {
                updateShadowTask.cancel(true);
                updateShadowTask = null;
            }
            updateShadowTask = new UpdateShadowTask();
            updateShadowTask.setThingName(StaticDataSave.thingName);
            float desiredValue;
            if (TextUtils.isEmpty(et_temperature.getText().toString())) {
                desiredValue = 0F;

            } else {
                desiredValue = Float.parseFloat(et_temperature.getText().toString().trim());
            }
            String newState = String.format("{\"state\":{\"desired\":{\"temperature\":%f}}}", desiredValue);
        ...
        if (StaticDataSave.thingName == null) {
            ...
            if (StaticDataSave.readyFlag == false) {
                ...
                if (waitDialog == null) {
                    ...
                    waitDialog.setCancelable(false);  //add in v2.3.15
                }
    ...
    public void onBackPressed() {
        ...
        if (terminateDialog == null && ((Activity) mContext).hasWindowFocus()) {
            ...
            terminateDialog.setCancelable(false);  //add in v2.3.15
        }
    ...
    private void initMainResource() {
        ...
        if (layerMainKey != null) {
            //[[modify in v2.4.12 because it was deprecated.
            //layerMainKey.setBackground(getResources().getDrawable(R.drawable.gradient_list));
        ...
        //[[remove in v2.4.11
        //progressingConnecting.setIndicatorColor(getResources().getColor(R.color.blue3));
        //]]
        ...
        if (progressingOpenClose != null) {
            //[[remove in v2.4.11
            //progressingOpenClose.setIndicatorColor(getResources().getColor(R.color.white));
            //]]
            //[[change in v2.3.17
            //progressingOpenClose.setVisibility(View.INVISIBLE);
            //[[change in v2.4.11
        ...
        //[[add in v2.3.19 for update shadow example
        /*et_temperature = findViewById(R.id.et_desiredTemperature);
        et_temperature.addTextChangedListener(new TextWatcher() {
        ...
        btn_setTemperature = findViewById(R.id.btn_setTemperature);
        btn_setTemperature.setEnabled(false);

        btn_setTemperature.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                setTemperature();
            }
    ...
    /*private void updateSetButtonState() {
        if (TextUtils.isEmpty(et_temperature.getText().toString())) {
            btn_setTemperature.setEnabled(false);
            btn_setTemperature.setBackgroundColor(mContext.getResources().getColor(R.color.light_gray));
        } else {
            btn_setTemperature.setEnabled(true);
            btn_setTemperature.setBackgroundColor(mContext.getResources().getColor(R.color.blue3));
        }
    }*/
    //]]
            ...
            if (isMqttConnected && isDeviceConnected) {
                ...
                if (buildOption.AUTO_TEST == true) {
                    ...
                    if (autoTestDialog == null) {
                        ...
                        autoTestDialog.show();
                        autoTestDialog.setCanceledOnTouchOutside(false);
                        autoTestDialog.setCancelable(false);  //add in v2.3.15
                    }
    ...
    private void updateBatteryUI(final float value) {
        //[[add in v2.4.12 for SMARTLOCK_BOARD
        if (buildOption.DEVICE == buildOption.SMARTLOCK_BOARD) {
    ...
    public void showUpdatingDialog() {
        ...
        handler.post(new Runnable() {  //add handler in v2.3.14
            @Override
            public void run() {
                if (updatingDialog == null) {
                    ...
                    updatingDialog.setCancelable(false);  //add in v2.3.15
                }
            }
        });
