...
public class MonitorService extends Service {
    ...
    private void initMqtt() {
        ...
        //[[remove in v2.3.16
        /*
        AWSIoTDoorActivity.keystorePath = getFilesDir().getPath();
        MyLog.i("keystorePath =  "+ AWSIoTDoorActivity.keystorePath);
        AWSIoTDoorActivity.keystoreName = KEYSTORE_NAME;
        AWSIoTDoorActivity.keystorePassword = KEYSTORE_PASSWORD;
        AWSIoTDoorActivity.certificateId = CERTIFICATE_ID;

        // To load cert/key from keystore on filesystem
        try {
            if (AWSIotKeystoreHelper.isKeystorePresent(AWSIoTDoorActivity.keystorePath, AWSIoTDoorActivity.keystoreName)) {
                if (AWSIotKeystoreHelper.keystoreContainsAlias(AWSIoTDoorActivity.certificateId, AWSIoTDoorActivity.keystorePath,
                        AWSIoTDoorActivity.keystoreName, AWSIoTDoorActivity.keystorePassword)) {
                    MyLog.i("Certificate " + AWSIoTDoorActivity.certificateId + " found in keystore - using for MQTT.");
                    // load keystore from file into memory to pass on connection
                    AWSIoTDoorActivity.clientKeyStore = AWSIotKeystoreHelper.getIotKeystore(AWSIoTDoorActivity.certificateId, AWSIoTDoorActivity.keystorePath, AWSIoTDoorActivity.keystoreName, AWSIoTDoorActivity.keystorePassword);
                    connectMqtt();
                } else {
                    MyLog.i("Key/cert " + AWSIoTDoorActivity.certificateId + " not found in keystore.");
                }
            } else {
                MyLog.i("Keystore " + AWSIoTDoorActivity.keystorePath + "/" + AWSIoTDoorActivity.keystoreName + " not found.");
            }
        } catch (Exception e) {
            MyLog.e("An error occurred retrieving cert/key from keystore. >>>"+e);
        }

        if (AWSIoTDoorActivity.clientKeyStore == null) {
            ...
            new Thread(new Runnable() {
                ...
                public void run() {
                    try {
                        ...
                        AWSIotKeystoreHelper.saveCertificateAndPrivateKey(AWSIoTDoorActivity.certificateId,
                        ...
                        // load keystore from file into memory to pass on connection
                        AWSIoTDoorActivity.clientKeyStore = AWSIotKeystoreHelper.getIotKeystore(AWSIoTDoorActivity.certificateId,
                                AWSIoTDoorActivity.keystorePath, AWSIoTDoorActivity.keystoreName, AWSIoTDoorActivity.keystorePassword);
                        ...
                        AWSIoTDoorActivity.mIotAndroidClient.attachPrincipalPolicy(policyAttachRequest);
    ...
    private void connectMqtt() {
        ...
        //if (AWSIoTDoorActivity.clientId != null) {  //remove in v2.3.16
            try {
                //[[modify in v2.3.16
                //AWSIoTDoorActivity.mqttManager.connect(AWSIoTDoorActivity.clientKeyStore, new AWSIotMqttClientStatusCallback() {
    ...
    private void sensorStatusUpdated(String doorlockStatusState) {
        ...
        if (String.valueOf(ds.state.reported.temperature) != null) {
            ...
            if (intTemperature >= 100) {
                ...
                //sendEvent(AWSEVENT.E_SHOW_TEMPERATURE_ALERT_MONITOR);
                SimpleDateFormat sfd = new SimpleDateFormat("yyyy-MM-dd hh:mm:ss aaa");
    ...
    private class SubScribeAsyncTask extends AsyncTask<Void, Void, Void> {
        ...
        protected Void doInBackground(Void... params) {
                ...
                try {
                    ...
                    //final String subTopic = SUBSCRIBE_TASK_TOPIC;
                    StaticDataSave.saveData = getSharedPreferences(StaticDataSave.mSharedPreferencesName, Context.MODE_PRIVATE);
                    StaticDataSave.thingName = StaticDataSave.saveData.getString(StaticDataSave.thingNameKey, null);
    ...
    void startForegroundService() {
        ...
        PendingIntent pendingIntentHide = PendingIntent.getBroadcast(this, 1, intentHide, PendingIntent.FLAG_CANCEL_CURRENT);
